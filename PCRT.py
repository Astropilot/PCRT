        ...
        if pos == -1:
            ...
            if data[pos + 8 :] != "":
                ...
                while True:
                    msg = Termcolor(
                        "Notice",
                        "Try extracting them in: <1>File <2>Terminal <3>Quit [default:3] ",
                    )
                    choice = input(msg)
                    if choice == "1":
...
Version: %s
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="don't show the banner infomation"
    )
